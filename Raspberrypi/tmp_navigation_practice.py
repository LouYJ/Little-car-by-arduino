...
from new_cross_navigation import cross_navigation
...
def navigation():
	...
	#espeak.synth('start')
	system("echo please input start point | festival --tts")
	...
	while (start_p > len(new_graph.V) or end_p > len(new_graph.V) or start_p <= 0 or end_p <= 0):
            ...
            #espeak.synth('invalid input!')
            #espeak.synth('start')
            system("echo invalid input | festival --tts")
	...
	while (check_point_area(cur_pos_x, cur_pos_y, new_graph.V[path[length-1]]) == 0 and ok_cnt < length):
		...
		system("echo direction is " + str(round(cur_dir,2)) + " | festival --tts")
	...
	#espeak.synth('Reach destination !')
...
def run_navigation():
	...
	system("echo welcome to use our navigation system| festival --tts")
	#navigation()
	
	
	system("echo if the destination crosses any building | festival --tts")
	cross_or_not = int(input('cross any building ? YES - 1  NO - 0'))
	if (cross_or_not == 1):
		cross_navigation()
	else:
		navigation()
...
if __name__ == '__main__':
	navigation()
	system("echo welcome to use our nativation system| festival --tts")
	'''
	system("echo if the destination crosses any building | festival --tts")
	cross_or_not = int(input('cross any building ? YES - 1  NO - 0'))
	if (cross_or_not == 1):
